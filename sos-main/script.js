...
function desenharCirculosMenores() {
    ...
    const textos = ['Início', 'Adicionar', 'Selva', 'Deserto', 'Oceano', 'Montanha', 'Savana', 'Caverna'];
...
// Array com as URLs para redirecionamento
const urls = [
    'principal.html',
    'pagina2.html',
    'pagina3.html',
...
// Loop para criar círculos menores
for (let i = 0; i < 5; i++) {
    const circuloMenor = document.createElement('div');
    circuloMenor.className = 'circulo-menor';
    
    // Adicionando funcionalidade de clique para redirecionar para a URL correspondente
    circuloMenor.addEventListener('click', () => {
        window.location.href = urls[i]; // Redireciona para a URL específica
    });

    document.body.appendChild(circuloMenor);
}
